...
export default function Surprise() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-pink-100 to-white text-gray-800 flex flex-col items-center justify-center px-4 py-8 relative overflow-hidden">
      {!showMessage ? (
        <>
          <button
            onClick={handleClick}
            className="text-3xl md:text-5xl font-bold bg-pink-600 px-6 py-3 rounded-2xl shadow-lg hover:bg-pink-500 transition fade"
          >
            {currentStep.label}
          </button>
          ...
          <img
            ...
            className="mt-8 rounded-xl shadow-xl w-full max-w-md fade"
          />

          <p className="mt-6 text-center text-lg md:text-xl max-w-xl fade">
            {currentStep.text}
          </p>
        </>
      ) : (
        <div className="text-center fade">
          <img
            src="/images/6.jpg"
            alt="Final"
            className="rounded-xl shadow-xl w-full max-w-md mb-6 fade"
          />
          <h2 className="text-3xl md:text-4xl text-pink-400 font-bold mb-4 fade">
            I love you very much, my woman.<br />
            A lifetime into years together 💖
          </h2>
          <p className="text-lg md:text-xl max-w-2xl mx-auto text-gray-600 fade">
